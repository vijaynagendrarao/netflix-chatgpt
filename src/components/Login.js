        ...
        if (!isSignIn) {
            createUserWithEmailAndPassword(auth, email.current.value, password.current.value)
                .then((userCredential) => {
                    ...
                    updateProfile(user, {
                        displayName: name.current.value, photoURL: USER_PROFILE_URL
                    }).then(() => {
                        // Profile updated!
    ...
    return (
        <div>
            ...
            <form onSubmit={(e) => { e.preventDefault(); }} className="w-3/12 absolute p-12 bg-black my-36 mx-auto right-0 left-0 text-white rounded-lg bg-opacity-90">
                ...
                {!isSignIn && (<input ref={name} type="text" placeholder="Full Name" className="p-2 my-2 w-full bg-gray-500 rounded-lg"></input>)}
                <input ref={email} type="text" placeholder="Email Address" className="p-2 my-2 w-full bg-gray-500 rounded-lg"></input>
                <input ref={password} type="password" placeholder="Password" className="p-2 my-2 w-full  bg-gray-500 rounded-lg"></input>
                <p className="text-red-500 font-bold text-lg py-2">{validationMessage}</p>
                <button className="p-4 my-6 bg-red-700 w-full rounded-lg font-bold" onClick={handleButtonClick}>{isSignIn ? "Sign In" : "Sign Up"}</button>
